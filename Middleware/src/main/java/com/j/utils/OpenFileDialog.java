	...
	static class FileSelectView extends ListView implements OnItemClickListener {
		...
		public FileSelectView(Context context, int dialogid,
			...
			if (imagemap == null) {
				...
				imagemap.put(OpenFileDialog.sRoot, R.drawable.folder_green); // 根目录图标
				imagemap.put(OpenFileDialog.sParent, R.drawable.format_up); // 返回上一层的图标
				imagemap.put(OpenFileDialog.sFolder, R.drawable.format_folder); // 文件夹图标
				// images.put("*", android.R.drawable.ic_media_rew); // wav文件图标
				imagemap.put(OpenFileDialog.sEmpty, R.drawable.format_unkown);
			}
			...
			for (File file : files) {
				if (file.isDirectory() && file.listFiles() != null) {
					...
					if (TextUtils.isEmpty(suffix)
							|| (!TextUtils.isEmpty(sf) && suffix.contains(sf))) {
						Map<String, Object> map = new HashMap<String, Object>();
						map.put("name", file.getName());
						map.put("path", file.getPath());
